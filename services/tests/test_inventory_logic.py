...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "../inventory-service")))
...
class TestCostCalculator:
    def test_calculate_cost_standard(self):
        # 100g * 0.15 * 1.2
        cost = Calculator.calculate_cost(weight_grams=100.0, price_per_gram=0.15, markup_percent=1.2)
        assert cost == 18.0

    def test_calculate_cost_zero(self):
        cost = Calculator.calculate_cost(weight_grams=0.0, price_per_gram=5.0, markup_percent=1.5)
        assert cost == 0.0

    def test_calculate_cost_negative_values(self):
        with pytest.raises(ValueError, match="Вес не может быть отрицательным"):
            Calculator.calculate_cost(weight_grams=-10.0, price_per_gram=5.0, markup_percent=1.5)
        
        with pytest.raises(ValueError, match="Расценки/наценка не могут быть отрицательными"):
            Calculator.calculate_cost(weight_grams=100.0, price_per_gram=-0.5, markup_percent=1.5)
...
class TestInventoryStock:
    ...
    def test_reserve_material_success(self):
        ...
        res_id = self.stock.reserve_material("order-1", 1, 250.0)
