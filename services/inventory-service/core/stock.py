...
class InventoryStock:
    ...
    def get_available(self, material_id: int) -> float:
        ...
        reserved = sum(
            res["amount"] for res in reservations if res["material_id"] == material_id
        )
    ...
    def add_material(self, material_id: int, amount_grams: float) -> float:
        ...
        if self.db.select("inventory", str(material_id)):
            ...
            self.db.insert(
                "inventory",
                str(material_id),
                {"material_id": material_id, "total_grams": new_total},
            )
    ...
    def confirm_reservation(self, order_id: str):
        ...
        self.db.update(
            "inventory",
            mat_id_str,
            {"total_grams": round(current_stock - res["amount"], 2)},
        )
